...
public class bank{
    ...
    public boolean sign_up(account acc ,String bank_id,String bank_password , String user_name , String bank_name , int user_money ){
    ...
    public String[] noti_bank(String bank_id, String bank_name , int so_tien, String type){
        switch (type){
            case "chuyen":
                String content ="Ban da " + type +" "+ so_tien + " USD vao so tai khoan " + bank_id +" thuoc ngan hang " +bank_name;
                DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
                LocalDateTime now = LocalDateTime.now();  
                String[] myarr = {(String) dtf.format(now),content};
                return myarr;
            case "nhan":
                String content1 ="Ban da " + type+ " " + so_tien + " USD tu so tai khoan " + bank_id +" thuoc ngan hang " +bank_name;
    ...
    public void send_noti_to_pc(){
        try {
            Desktop.getDesktop().open(new File("home/cr4b/bank"));
    ...
    public String[] create_history(int so_tien, String user_id,String user_bank,String type){
        ...
        switch (type){
            case "chuyen":
                DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
                LocalDateTime now = LocalDateTime.now();
                String  content = "ban da " + " " + type1 + " " + so_tien + " " + " vao TK " + user_id + " ngan hang " +user_bank;
                String[] myarr = {dtf.format(now),content};
                return myarr;
            case "nhan":   
                DateTimeFormatter dtf1 = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
                LocalDateTime now1 = LocalDateTime.now();
                String  content1 = "ban da " + " " + type2 + " " + so_tien + " " + " tu TK " + user_id + " ngan hang " +user_bank;
